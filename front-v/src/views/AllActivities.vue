<template>
  <div class="act_cont">
    ...
    <input
      ...
      v-model="searchQuery"
    />
    <h1 class="mt-5 mb-5 text-center">All Activities</h1>
    <div class="containercards">
      <div class="card" v-for="activity in resultQuery" :key="activity.id">
        ...
        <div class="card__body">
          ...
          <div class="user"></div>
          <!-- pictures API Tripadvisor-->
          <img
            v-if="activity.location_id != null"
            src="../assets/images/Rio-carnaval.jpg"
            alt="rio_carnaval"
            style="height: 350px; width: 350px"
          />
          <!-- -->
          ...
          <div class="card__body">
            ...
            <div class="card__footer">
              <div>
                <router-link
                  :to="{ name: 'CommentsRates', params: { id: activity.id } }"
                >
                  <button
                    href="#"
                    type="button"
      ...
      <div class="card" v-for="activ in resultQuery3" :key="activ.location_id">
        ...
        <div class="card__body">
          <h3>{{ activ.name }}</h3>
          <p class="city">
            City : <span>{{ activ.address_obj.city }}</span>
          </p>
      ...
      <div class="card" v-for="parc in resultQuery2" :key="parc.location_id">
        <img
          v-if="parc.address_obj.city === 'Barcelona'"
          src="../assets/images/parc-guell.jpg"
          alt="rio_carnaval"
          style="height: 350px; width: 350px"
        />
        <img
          v-if="parc.address_obj.city != 'Barcelona'"
          src="../assets/images/city_park.jpg"
          alt="rio_carnaval"
          style="height: 350px; width: 350px"
        />
        <div class="card__body">
          <h3>{{ parc.name }}</h3>
          <p class="city">
            City : <span>{{ parc.address_obj.city }}</span>
          </p>
      ...
      <div class="card" v-for="hotel in resultQuery4" :key="hotel.location_id">
        ...
        <div class="card__body">
          <h3>{{ hotel.name }}</h3>
          <p class="city">
            City : <span>{{ hotel.address_obj.city }}</span>
          </p>
      ...
      <div
        class="card"
        v-for="museum in resultQuery5"
        :key="museum.location_id"
      >
        <img
        ...
        <div class="card__body">
          <h3>{{ museum.name }}</h3>
          <p class="city">
            City : <span>{{ museum.address_obj.city }}</span>
          </p>
...
export default {
  ...
  mounted() {
    ...
    axios.get(`http://localhost:3300/search/` + "restaurants").then((res) => {
    ...
    axios.get(`http://localhost:3300/search/` + "parc").then((res) => {
      console.log(res.data.data);
      localStorage.setItem("locationId", res.data.data[0].location_id);
      console.log(res.data.data[0].location_id);
      this.parcs = res.data.data;
    });
    axios.get(`http://localhost:3300/search/` + "hotel").then((res) => {
      console.log(res.data.data);
      localStorage.setItem("locationId", res.data.data[0].location_id);
      console.log(res.data.data[0].location_id);
      this.hotels = res.data.data;
    });
    axios.get(`http://localhost:3300/search/` + "museum").then((res) => {
      console.log(res.data.data);
      localStorage.setItem("locationId", res.data.data[0].location_id);
      console.log(res.data.data[0].location_id);
